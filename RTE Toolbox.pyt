...
class Toolbox(object):
    def __init__(self):
        ...
        self.tools = [CreatePIR, CalcMountRatio, SelectTest]
...
class CalcMountRatio(object):
    def __init__(self):
        """Define the tool (tool name is the name of the class)."""
        self.label = "Calculate Mount Ratio"
        self.description = ""
        self.canRunInBackground = False
...
class SelectTest(object):
    def __init__(self):
        """Define the tool (tool name is the name of the class)."""
        self.label = "Select Test"
        self.description = ""
        self.canRunInBackground = False

    def getParameterInfo(self):
        """Define parameter definitions"""
        param0 = arcpy.Parameter("inputFeatureClass","Municipality Dataset","Input", ["DEFeatureClass", "GPString"],"Required")
        param1 = arcpy.Parameter("statusField","Status or Installation","Input", "Field","Required")
        param1.filter.list = ['Text']
        param1.parameterDependencies = [param0.name]  
    ...
    def execute(self, parameters, messages):
        ...
        inputFeatureClass = parameters[0].valueAsText
        statusField = parameters[1].valueAsText
        queryInstallation = statusField + " = 'Installed'"
        arcpy.MakeFeatureLayer_management (inputFeatureClass, "inputFeatureClassOut")
        arcpy.SelectLayerByAttribute_management("inputFeatureClassOut", "NEW_SELECTION", queryInstallation)